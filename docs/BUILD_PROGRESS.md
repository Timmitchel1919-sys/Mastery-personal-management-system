...
| **In-progress work** | — (build order finished). **Owner setup for CI deploy**: add the repo secrets in `docs/DEPLOYMENT.md` §3 (`FIREBASE_SERVICE_ACCOUNT`, `NEXT_PUBLIC_FIREBASE_*`); provision a real `staging` project. **Post-Blaze**: Cloud Functions deploy + App Check enforcement (ADR-0029) + `web-vitals` sink (`docs/PERFORMANCE.md` §6). **i18n backlog** (ADR-0027): `SearchTrigger` / `ModulePlaceholder` / `SectionLanding` migrated 2026-09-06 (no test churn); still open — breadcrumbs + `EmptyState`/`ErrorState`/`LoadingState` defaults + per-domain feature-view copy, all blocked on putting ~30 view-test files on `renderWithIntl` in one commit. **Branding (2026-09-06):** the gold "M" mark now sits in the sidebar header, and `/login` got the full lockup + a frosted `.mastery-glass-card` over a `.mastery-auth-backdrop` gradient (see `docs/CHANGELOG.md`) — first use of `next/image` in the app. **Sidebar nav (2026-09-06):** Plan/Focus/Act/Grow/Analytics are collapsible dropdowns (tree-style children, active section open by default); the theme toggle is off the topbar (still on Settings + ⌘K). **Brand pivot (2026-09-06):** full Gold/Obsidian/Ivory/Slate design system (ADR-0033) replaces the Layer 2 indigo palette — light is the primary theme; new `/` marketing landing page; auth screens rebuilt with the real logo + glass. `docs/DESIGN_SYSTEM.md` still describes the old palette — owed a catch-up pass. |
| **Test status** | ✅ app: `vitest run` — 139 files, 736 tests (`tests/unit/bundle-budget.test.ts` — script logic + real-build-within-budget — added; `ci-workflow.test.ts` asserts the CI `analyze` step; 2 new `a11y.test.tsx` cases; `sidebar-nav.test.tsx` +3 cases for the dropdown toggle). ✅ coverage: `npm run test:coverage` — v8, baseline ≈ 54% stmts / 57% lines / 60% branches / 49% funcs; thresholds pinned at that floor (CI ratchet). ✅ analyze: `npm run analyze` — bundle within budget (~818 KiB total JS gzip / 900 · ~192 KiB largest chunk / 240; the small rise is `next/image`'s runtime, first used in the branding change). ✅ e2e: `npm run test:e2e` — `tests/e2e/` (5 specs → 22 runs) — **written; runs in CI** (sandbox has no Playwright browsers / emulator). ✅ rules: `npm run test:rules` — 3 files, ~45 tests (unchanged; emulator restriction). ⚠️ integration: `npm run test:integration` — 34 files **written**; unchanged. ✅ functions: `vitest run` — 16 files, 98 tests (unchanged — Layer 23 touches no Cloud Function). |
...
| **Git status** | Commit-and-push per layer (`CLAUDE.md` §10); §10.1 — mandatory end-of-session commit + push + deploy. **CI/CD wired (Layer 22)**: `.github/workflows/ci.yml` (app / functions / emulator / e2e → deploy-on-`main`) + `pr-preview.yml` + `dependabot.yml`; the `app` job now also runs the Layer 23 bundle-budget gate. Needs the owner to add repo secrets before the CI deploy job can run (manual §2a release stays the fallback). **Layers 9D → 23 merged to `origin/main` via fast-forward on 2026-09-06** (29 commits; `2a75d25`). The local `main` in the non-worktree checkout still needs a `git pull`. Post-build-order work (i18n chrome) continues on the same worktree branch, fast-forward-pushed to `main`. |
