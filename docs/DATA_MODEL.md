...
users/{uid}/recoveryAccountabilityPartners/{partnerId}       an opt-in grant: goalId, partnerEmail, partnerLabel, scope (streak-only|status-only|check-in-completed|selected-summary|custom-limited-access), customFields[], includeSetbackCount, sendCheckInReminders, expiresAt, revokedAt — **written only by `configureAccountabilityPartner`** (rules reject a direct client write). The owner reads their own list; the partner NEVER reads Firestore — they call `getAccountabilityProjection`, which returns only the scoped projection (Layer 15F)
